...
export function analyzeQueryComplexity(query: string): QueryComplexity {
  ...
  const hasMultipleConcepts =
    /\b(and|or|versus|vs|compared|difference|between)\b/gi.test(query);

  // Check for complex sentence structure
  const hasSubClauses = (query.match(/[,;]/g) || []).length > 1;
  const hasQuestionWords = (
    query.match(/\b(what|who|which|when|where|why|how)\b/gi) || []
  ).length;
...
export function getSimilarityThreshold(
  ...
  const thresholds: Record<QueryComplexity, Record<QueryType, number>> = {
    [QueryComplexity.SIMPLE]: {
      [QueryType.FACTUAL]: 0.3, // Specific queries
    ...
    [QueryComplexity.MODERATE]: {
      ...
      [QueryType.ANALYTICAL]: 0.2, // Standard explanations
...
export function analyzeQuery(query: string): QueryAnalysis {
  ...
  const hasKeywords =
    /\b(what|who|which|when|where|why|how|verse|chapter|krishna|arjuna)\b/i.test(
      query,
    );
 ...
 */
export function getRetrievalCount(
  complexity: QueryComplexity,
  finalCount: number,
): number {
  const multipliers = {
